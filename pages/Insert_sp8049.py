...
if 'data' in st.session_state:
    if st.session_state.data['status'] == 200:
        if st.session_state.data['netid'] is not None:
            ...
            ''').to_pandas()
            #where clause removed to allow all crosswalks to be inserted (temporary)
                        ...
                        if x.status_code == 200:
                            ...
                            if y.json()['importJobState'] == 'COMPLETED':
                                ...
                                if z.status_code == 200:
                                    ...
                                    df['targetCodeSets'] = df['targetCodeSets'].apply(lambda x: x[0] if isinstance(x, list) and x else {})
                                    df['REFDATA_TARGET_CODESET']= 'Authoritative School Codeset'
